...
module ApplicationHelper
  def tabs(tabs = nil)
    tabs ||= controller_path.match?(/admin/) ? CRM::Tabs.admin : CRM::Tabs.main
    if tabs
      ...
      raise CRM::MissingSettings, "Tab settings are missing, please run <b>rake ffcrm:setup</b> command."
    end
  end
  ...
  def template_for_current_view
    CRM::ViewFactory.template_for_current_view(controller: controller.controller_name,
  ...
  def view_buttons
    views = CRM::ViewFactory.views_for(controller: controller.controller_name,
                                              action: show_or_index_action)
