...
module CRM
  module I18n
    ...
    # Scan config/locales directory for CRM localization files
    # (i.e. *_crm.yml) and return locale part of the file name.
    # We can't use ::I18n.available_locales because rails provides it's own
    # translations too and we only want the locales that CRM supports.
    #----------------------------------------------------------------------------
    def locales
      @@locales ||= ::I18n.load_path.grep(/_crm\.yml$/).map { |path| Pathname.new(path).basename.to_s.match(/(.*)_crm\.yml/)[1] }.uniq
    end

    # Return a hash where the key is locale name, and the value is language name
    # as defined in the locale_crm.yml file.
    #----------------------------------------------------------------------------
    def languages
...
ActionView::Base.include CRM::I18n
ActionController::Base.include CRM::I18n
