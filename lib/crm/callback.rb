...
module CRM
  module Callback
    @@classes   = []  # Classes that inherit from CRM::Callback::Base.
    ...
    # Adds a class inherited from from CRM::Callback::Base.
    #--------------------------------------------------------------------------
    def self.add(klass)
      @@classes << klass
    end
    ...
    # This makes it possible to call hook() without CRM::Callback prefix.
    ...
    module Helper
      def hook(method, caller, context = {}, &block)
        ...
        if is_view_hook
          hooks = CRM::Callback.view_hook(method, caller, context)
          ...
          if hooks[:replace].empty?
            view_data << if block_given?
                           ...
                           # legacy view hooks
                           CRM::Callback.hook(method, caller, context)
              end
          else
...
ActionView::Base.include CRM::Callback::Helper
ActionController::Base.include CRM::Callback::Helper
